...
//Account 
async function getUserInstaPages(id,token) {

  console.log('instagram service::getUserPages  accessToken ', id);
  try {
    // fields: "id",
    var userID_params = {
    ...
    console.log("instagram get pages info by id",me)
...
async function getUserPages(accountId, token) {
    console.log('instagram service::getUserPages  accessToken ', token);
    try {
      // fields: "id",
      var userID_params = {
...
async function getPageVideos(pageId, token) {
  ...
  try {
    var params = {
      fields:'id,description,updated_time,created_time,source,icon,permalink_url,published,title,embeddable,video_insights{id,name,values,title,period,description,description_from_api_doc},thumbnails',
      access_token: token
    };
    let me = await request({
...
async function refreshUserToken(token) {
  ...
  console.log('::refreshUserTokenIG  accessToken ', token);
  try {
    let me = await request({
      url: 'https://graph.facebook.com/oauth/access_token',
      qs: params
    });
    console.log('::refreshUserTokenIG  response  ', me);
    return Promise.resolve(JSON.parse(me));
  } catch (err) {
    console.error('::refreshUserTokenIG error <- ', err);
    return Promise.reject(err);
  }
}
//Audience
...
const facebookService = {
  ...
  getUserInstaPages:getUserInstaPages,
