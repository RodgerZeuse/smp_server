...
async function twiterImagePost(
  access_token_key,
  access_token_secret,
  status,
  img
  ...
  try {
    let twit = await client.post(
      "https://upload.twitter.com/1.1/media/upload.json",
      {
        media: [
          fs.createReadStream(
            "/Users/fistixdev/workspace/SocialMediaPoster/server/server/local-storage/pics/" +
              img
          )
        ]
      }
    ...
    if (twit.media_id) {
      ...
      try {
        let twitt = await client.post("statuses/update", {
          status: status,
          media_ids: media_id
        });
        console.log(twitt);
...
async function twiterVideoPost() {
  ...
  client.post(
    "https://upload.twitter.com/1.1/media/upload.json",
    { command: "INIT", total_bytes: "791000", media_type: "video/mp4" },
...
async function _twitterVideoPub(
  access_token,
  access_token_secret,
  status,
  video
  ...
  return new Promise(function(resolve, reject) {
    ...
    const PATH = path.join(
      `/Users/fistixdev/workspace/SocialMediaPoster/server/server/local-storage/pics/${video}`
    );
