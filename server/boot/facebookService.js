...
const app_secret = "8eadbc11ea1959dede6da8b2191ad273";
const app_id = "612367679148277";
...
async function getUserBusinesses(accountId, token) {
  ...
  try {
    var params = {
      fields:
        "payment_account_id,permitted_roles,profile_picture_uri,primary_page,name,link,id,vertical",
      access_token: token
    };
    let me = await request({
...
async function getBusinessOwnedPages(businessId, token) {
  ...
  try {
    ...
    var params = {
      fields:
        "name,link,engagement,page_token,access_token,ad_campaign,about,instagram_business_account,connected_instagram_account,fan_count",
      access_token: token
    };
    let me = await request({
...
async function getBusinessOwnedAdAccounts(businessId, token) {
  ...
  try {
    var params = {
      fields:
        "name,account_id,business,age,agency_client_declaration,account_status,amount_spent,balance,business_name,is_personal,id",
      access_token: token
    };
    let me = await request({
...
async function getPageDetails(pageId, token) {
  ...
  try {
    var params = {
      fields:
        "name,link,engagement,access_token,page_token,ad_campaign,about,instagram_business_account,connected_instagram_account,fan_count",
      access_token: token
    };
    let me = await request({
...
async function getPageVideos(pageId, token) {
  ...
  try {
    var params = {
      fields:
        "id,description,countViews,updated_time,created_time,source,icon,permalink_url,published,title,embeddable,video_insights{id,name,values,title,period,description,description_from_api_doc},thumbnails",
      access_token: token
    };
    let me = await request({
...
//Audience
async function createFBPageAudienceWithAdAccount(
  name,
  subtype,
  description,
  pageId,
  ...
  console.log(
    "::createFBPageAudienceWithAdAccount  params ",
...
const facebookController = {
  async refreshToken(req, res) {
    ...
    try {
      ...
      accounts.forEach(async ac => {
        let userToken = ac.lengthySession
          ? ac.lengthySession.access_token
          : ac.session.accessToken;
        ...
        channelSources.forEach(async cs => {
          let pageToken = cs.lengthySession
            ? cs.lengthySession.access_token
            : cs.source.access_token;
  ...
  async getUserBusinesses(req, res) {
    console.log(
      "::getUserBusinesses userId, accountId, accessToken \n \t",
      req.query.userId,
      req.query.accountId,
    ...
    try {
      ...
      let resp = await getUserBusinesses(
  ...
  async getBusinessDetails(req, res) {
    console.log(
      "::getBusinessOwnedPages accountId, accessToken \n \t",
    ...
    try {
      ...
      let resp = await getBusinessOwnedPages(
        req.params.businessId,
        account.session.accessToken
      );
      ...
      account.businesses = resp.data;
      account.save();
      resp = await getBusinessOwnedAdAccounts(
        req.params.businessId,
        account.session.accessToken
      );
  ...
  async getPageDetails(req, res) {
    ...
    try {
      ...
      let channelSource = await ChannelSource.findById(
        req.params.channelSourceId
      );
      console.log("::getBusinessDetails channelSource \n \t", channelSource);
      let resp = await getPageDetails(
        channelSource.source.id,
        channelSource.source.access_token
      );
      console.log("::getBusinessOwnedPages resp => ", resp);
  ...
  async getUserPages(req, res) {
    console.log(
      "::getUserPages accountId, accessToken \n \t",
      req.query.accountId,
      req.query.accessToken
    );
    try {
  ...
  async createAudienceWithAdAccount(req, res) {
    ...
    try {
      let resp = await createAudienceWithAdAccount(
        req.body.name,
        req.body.subtype,
  ...
  async shareAudienceWithAdAccount(req, res) {
    console.log("::createAudienceWithAdAccount");
    try {
      let resp = await shareAudienceWithAdAccount(
        req.body.audienceId,
        req.body.adAccountIds,
        req.query.accessToken
      );
  ...
  async unshareAudienceWithAdAccount(req, res) {
    console.log("::unshareAudienceWithAdAccount");
    try {
      let resp = await unshareAudienceWithAdAccount(
        req.body.audienceId,
        req.body.adAccountIds,
        req.query.accessToken
      );
  ...
  async deleteAudienceFromAdAccount(req, res) {
    ...
    try {
      let resp = await deleteAudienceFromAdAccount(
        req.body.audienceId,
        req.query.accessToken
      );
  ...
  async createCustomAudience(req, res) {
    ...
    request(
      {
        url:
          "https://graph.facebook.com/v2.12/act_" +
  ...
  async createFBPageAudience(req, res) {
    ...
    try {
      ...
      let resp = await createFBPageAudienceWithAdAccount(
        req.body.name,
        "ENGAGEMENT",
        req.body.description,
        req.body.pageId,
